...
- **A project with zero Arc axes configured sends no response at all to `getAllPosition`** — confirmed by removing every axis and re-querying: complete silence for a full 3-second listening window, not an empty bundle. Client code cannot distinguish "query not yet answered" from "query answered with zero axes" by waiting for a response; it must use a timeout to infer the zero-axes case.
