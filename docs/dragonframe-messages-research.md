...
**The plan this suggests:** configure a set of axes once inside Dragonframe with channels 1–17 wired to match DragonMIDI's opinionated map (faders on 1–8, knobs on 9–16, jog wheel on 17), export that as a reusable `.arcx` template, and from then on just import it into any new Dragonframe project instead of manually re-entering channel/scale/absolute settings per axis each time. This direction is worth pursuing — it turns "configure 17 axes by hand" into a one-time template plus an import click per project.
