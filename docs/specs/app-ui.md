...
- [x] **UI-MAP-002**: The system shall permit editing the target type and target only on the 8 fader rows (`MAP-AXIS-004`, see `docs/specs/static-mapping.md`); knob, button, and jog-wheel rows shall display their target with no control to change it. A knob, Mute, or Solo row's displayed target may still vary based on its bank's fader state (`UI-MAP-012`) without introducing a control of its own.
...
- [x] **UI-MAP-012**: The system shall recompute a knob, Mute, or Solo row's displayed target every UI update tick from its bank's fader state: `stepPosition → {axis}` / `setZero → {axis}` / `setHome → {axis}` respectively while the bank's fader has a real axis name assigned (`MAP-BANK-001`/`MAP-BANK-002`/`MAP-BANK-003`), and its opinionated encoder-channel / encoder-reset target otherwise (`MAP-BANK-004`).
