...
- [x] **OSC-DISCOVER-004**: When a received datagram begins with the OSC bundle marker (`#bundle\0`), the system shall recursively decode its contained elements (each itself an OSC message or a nested bundle), subject to the bounds in `OSC-DISCOVER-010`/`OSC-DISCOVER-011`, rather than treating the datagram as a single non-bundle message.
- [x] **OSC-DISCOVER-010**: When decoding a bundle element, if that element's declared `int32` size is not strictly positive, or exceeds the number of bytes remaining in the buffer, the system shall raise rather than using the declared size to slice the buffer or advance the decode offset.
- [x] **OSC-DISCOVER-011**: When recursively decoding nested `#bundle` elements, the system shall raise if nesting would exceed a depth of 8, rather than recursing further; a failure from this spec or `OSC-DISCOVER-010` shall propagate to the datagram's top-level decode attempt, invalidating that datagram's entire decode per `OSC-DISCOVER-007`, not only the offending element.
- [x] **OSC-DISCOVER-012**: When a failure from `OSC-DISCOVER-010` or `OSC-DISCOVER-011` is caught at the datagram's top-level decode attempt, the system shall log it at debug level, unlike other decode failures covered by `OSC-DISCOVER-007`, which remain unlogged.
