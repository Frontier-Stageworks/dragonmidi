...
### EVID-004 — DM-004, opinionated-map synthesis correctness for the two bundled profiles

- **Remaining uncertainty:** does `build_opinionated_map`'s output, for the Studio's and nanoKONTROL2's declared `controls:` CC/channel numbers specifically, actually match what `MAP-TABLE-001/002/003/005` and `MAP-CONFIG-004/005/006/007/008` require — independent of what the function itself currently produces? This evidence design covers only the two bundled profiles' concrete declared values — it does not, and is not intended to, establish the same claim for arbitrary third-party `controls:` declarations, even though they run through the same shared helpers. See `property-register.md` `DM-004`'s second residual gap for that separately-tracked domain.
- **What passing would establish:** for the two bundled profiles' concrete declarations only, every synthesized entry (target OSC address/action, channel-match condition, continuous-vs-one-shot framing) matches a value computed by a human directly from the spec text and each profile's own declared CC list.
...
- **Residual gaps:** (1) evidence missing for the bundled-profile claim as stated — Type: Evidence missing. Disposition: Produce evidence. Routed to `evidence-handoff.md`. (2) generalization to arbitrary third-party `controls:` declarations is not covered by this evidence design at all — Type: Evidence missing. Disposition: Defer (see `property-register.md` `DM-004`).
...
- **Evidence purpose:** correctness.
- **Selected evidence — Primary:** Example testing (written in `tests/test_websocket_output.py`; **not confirmed by a passing run** — see Evidence state).
- **Claim-dimension discharge:** `E-Stop`/bare-trigger encode → `test_encode_bare_trigger_omits_operation_and_params`; ranged command encode → `test_encode_ranged_command_includes_operation_and_params`; delivery when connected → `test_send_is_delivered_to_the_connected_client`; no-connection drop → `test_send_with_no_active_connection_is_dropped_without_raising`, `test_send_before_start_is_dropped_without_raising`, `test_send_after_stop_is_dropped_without_raising`, `test_send_after_client_disconnects_is_dropped_without_raising`. All four dimensions have a targeted test in source; **none has been confirmed by a passing execution** — treat as covered-by-artifact, not covered-by-result, until the environment gap below is resolved.
- **Vacuity / discrimination:** not independently mutation-tested; encode tests assert exact JSON shape (discriminating by construction — any wrong field value fails the equality assertion), *if and when the suite runs*.
- **Evidence state (set):** Test artifact exists in source, confirmed correct by direct inspection. **Execution status: unverified** — the suite fails to import in this development environment (`websockets` module not installed), and there is no prior known-good run being relied on here. Do not record this as "Example-tested" without this qualifier in any downstream artifact.
- **Residual gaps:** environment-platform validation gap — `tests/test_websocket_output.py` fails to import in this development environment. This is the reason the property cannot currently be called established by a passing result; not a code defect, but a real block on the evidence's current validity. Type: Environment-platform validation gap. Disposition: Resolve through LID/design (install/pin the dependency in the dev environment) — routed to `evidence-handoff.md`.
- **Freshness:** stale — renewal needed; no execution has occurred to be "current" as of. Content confirmed present and correct by direct file inspection as of 2026-08-26 — inspection is not execution.
...
No property's *primary* claim has a Defer or Accept-risk disposition — every property above either has realized evidence for its primary claim (or, for `DM-007`, a written-but-unexecuted evidence artifact — see `EVID-007`), a specified evidence plan routed to `evidence-handoff.md`, or an explicit "Additional evidence: none justified" conclusion. Three properties carry a secondary Defer/Accept-risk gap without it affecting their primary claim's status: `DM-004`'s third-party-Controller-Profile generalization gap (Defer), `DM-008`'s doubly-failing-`release()` ordering (Defer), and `DM-EA-002`'s generalization-beyond-the-tested-configuration gap (Accept risk) — all three tracked in `evidence-handoff.md`/`property-register.md`, none listed here as a whole-property no-active-evidence-work case.
