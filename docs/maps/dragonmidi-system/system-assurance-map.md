...
| `mapping.py::build_opinionated_map` / `_fader_entries()`/`_knob_entries()`/`_mute_entries()`/`_shared_button_entries()` | Every Controller Profile's synthesized MIDI→OSC/keystroke/WebSocket table, both bundled profiles and every third-party config | If wrong, every control on every profile using the shared helpers is silently mis-mapped | `test_mapping_config_schema.py` exists but compares the function's output to constants derived from the same function — see Known fragile areas below. |
...
| WebSocket JSON commands (`E-Stop`, `select-AXn`, `jog-AXn`) | Dragonframe's WebSocket client, `{"input": ...}` shape | `E-Stop` is a motion-control emergency-stop function — a dropped or malformed send is a safety-relevant silent failure | Example tests for encoding/dispatch; no evidence yet characterizing delivery reliability under adapter failure (see Known fragile areas). |
...
| `build_opinionated_map` producing an incorrect entry for a bundled or third-party profile | A bug in `_fader_entries()`/`_knob_entries()`/`_mute_entries()`/`_shared_button_entries()`, or a config file whose `controls:` values don't mean what the author intended | Every control the shared helpers derive is silently wrong for every profile using them | All controls across all profiles (highest blast radius single point in the mapping layer) | `test_mapping_config_schema.py:76,81` — its comparison is circular (checks the function's output against constants that are themselves produced by the same function); large example-test suite exercises the resulting `OPINIONATED_MAP_STUDIO`/`NANOKONTROL2` behavior independently, giving some incidental protection. |
...
| `tests/test_mapping_config_schema.py` | `build_opinionated_map` output vs. `OPINIONATED_MAP_STUDIO`/`NANOKONTROL2`; Controller Profile validation error paths | Example tests — the two output-equality assertions are circular (see Known fragile areas) |
...
| `tests/test_websocket_output.py` | Encoding, bind, connection handling, send dispatch | Example tests (currently fails to import in this environment — `websockets` module missing; not a code defect) |
...
|---|---|---|
| `mapping.py::build_opinionated_map` / `OPINIONATED_MAP_STUDIO`/`_NANOKONTROL2` | `test_mapping_config_schema.py:76,81` asserts the function's output equals two constants that are themselves defined as `STUDIO_PROFILE.opinionated_map`/`NANOKONTROL2_PROFILE.opinionated_map`, both built by calling the identical function with the identical arguments (confirmed by direct code inspection) | The test cannot currently distinguish a correct opinionated map from a consistently-wrong one; the highest-blast-radius chokepoint in the mapping layer has no independent correctness evidence, only regression protection. |
| `osc_io.py::decode_osc_packet` malformed-bundle handling | Guards are mutation-tested (`DM-001`); the code's own comment history framed the guards as "preventing a hang," but direct testing shows the pre-guard code never actually hung on the tested inputs — the accurate justification is bounded recursion cost, not hang prevention | Claim wording in this register is deliberately independent of the code's own prior justification comment. |
| `test_listener_resends_discovery_query_on_rebind` | Intermittently flaky; confirmed via `git stash` to fail identically on the unmodified codebase | Pre-existing and unrelated to any claim in this package, but an assurance claim resting on this test's current green state would be unreliable without investigating the flake first. |

## Higher-level assurance goals

| Goal | Decomposes into (property IDs, once registered) |
|---|---|
| The bridge should not corrupt or misdirect Dragonframe's motion-control state even under malformed OSC input, a bad Preset Store file, or an incorrect third-party Controller Profile | TBD — Phase 2 |
| A control mapped to a safety-relevant WebSocket command (`E-Stop`) should reliably reach Dragonframe, or the operator should be able to tell when it hasn't | TBD — Phase 2 |
| Every bundled or third-party Controller Profile's synthesized opinionated map should actually match its intended per-control behavior, not merely stay self-consistent over time | TBD — Phase 2 |

## Open questions surfaced during mapping

- None require reconciliation before Phase 2 priorities can be trusted — the system's intent is well-documented in the HLD/LLDs/specs, and direct code/test inspection resolved the ambiguities that would otherwise block prioritization (e.g., whether the malformed-bundle guard's justification held up, whether the Preset Store's index bounds-check was already in place).
