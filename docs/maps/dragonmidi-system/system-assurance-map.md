...
| `decode_osc_packet` malformed/adversarial bundle | A crafted or corrupted UDP packet to DragonMIDI's listen port with `element_size <= 0` or deeply nested `#bundle` framing | Possible infinite loop (`offset` fails to advance) or unbounded recursion (stack exhaustion) in the listener thread | Low likelihood (any bug here needs a local, on-purpose or badly buggy sender) but a real hang path in a background thread with no `except`-guard at the `decode_osc_packet` entry point itself | `handle_datagram`'s broad `except Exception` catches raised exceptions but a hang (not raising) would not be caught by it |
