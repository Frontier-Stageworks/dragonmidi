...
| Permission pre-flight | None | Detect Accessibility/permission state before sending, surface a dedicated warning | No platform API queried is simpler than one that is; graceful failure on the actual `press`/`release` call already covers the "it didn't work" case, just without advance warning |
