...
- **Axis name selection is picker-only, not free text.** The user selects from the OSC Listener's currently-discovered axis list; there is no way to type an arbitrary/undiscovered name into a direct-axis mapping. A free-text field would let a typo produce a mapping that silently sends `gotoPosition` to a name Dragonframe doesn't recognize, with no way for DragonMIDI to detect the mistake (see Open Questions on stale references, below).
...
5. The picker-only axis-selection UI, and how the mapping view surfaces "never discovered yet" vs. "discovered, zero axes" (see `docs/llds/osc-io.md`'s equivalent open item), belong in `docs/llds/app-ui.md`'s Mapping View section, which has not yet been updated for this target type — a pending cascade, not a decision made here.
