...
This component is the one place MIDI meaning becomes Dragonframe meaning. It is a pure, stateless-as-possible translation: normalized MIDI event in, zero-or-one Dragonframe OSC message out. The default map mirrors the prototype's validated default table (`DragonMIDI-vibed/mappings.md`), which reflects behavior already validated in use rather than an unconstrained guess.

**Note (pending update):** this LLD still describes the map as hard-coded with no editor, MIDI-learn, or persisted presets. `docs/high-level-design.md` now specifies an editable mapping engine (add/edit/remove, MIDI-learn, custom OSC paths, save/load presets) defaulting to this same table. This LLD has not yet been revised to match; treat the sections below as the current-code baseline, not the target design.
