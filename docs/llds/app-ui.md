...
- **Controller Profile dropdown**, above the indicator rows: two options, "nanoKONTROL Studio" and "nanoKONTROL2" (`docs/llds/midi-input.md § Controller Profiles`). Defaults to nanoKONTROL Studio on launch, preserving the existing zero-config experience for anyone who never touches it. Changing the selection applies immediately (no Apply step, unlike the host/port fields below) — it disconnects the currently-connected device (if any), resets the Mapping Engine to the newly-selected profile's opinionated default map, and starts polling for the new profile's name match. This is a heavier action than the host/port fields' Apply, so it's a plain dropdown rather than needing a confirm step: switching controllers is inherently a "start over with a different device" action, and there is no partially-applied state to protect against the way there is with in-progress host/port text.
- When nanoKONTROL2 is selected, a short helper line appears beneath the dropdown: "Hold SET MARKER + CYCLE while powering on for CC mode" — a direct pointer to the one-time physical setup step DragonMIDI cannot detect or perform itself (`docs/llds/midi-input.md`'s documented-precondition treatment). No equivalent line is needed for the Studio, since its Native Mode handshake is automatic and any failure already surfaces via the MIDI indicator's *error* state.
...
| Controller Profile default on launch | nanoKONTROL Studio | Remember the last-selected profile; require an explicit choice before connecting | Preserves the existing zero-config experience for current users; matches "no persistence yet" pattern elsewhere in this LLD |
| nanoKONTROL2 setup guidance | A one-line helper hint under the dropdown ("Hold SET MARKER + CYCLE…") shown only when that profile is selected | No in-app guidance; leave it to external documentation only | The one-time physical setup step has no software-observable success/failure signal (`docs/llds/midi-input.md`), so a direct, low-cost hint reduces the "why isn't this working" confusion a silent precondition would otherwise cause |
...
8. **Controller Profile dropdown:** immediate-apply (no Apply step), defaults to nanoKONTROL Studio on launch, triggers a disconnect + Mapping Engine reset on change, and shows a one-line setup hint for the nanoKONTROL2's manual CC-mode power-on procedure. Mapping View rows for controls the active profile lacks (jog wheel, Scene button) are omitted entirely rather than shown disabled — see Decisions & Alternatives above.
