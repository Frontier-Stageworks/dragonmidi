...
- **Zero-axes detection is timeout-based, not response-based.** It isn't known whether Dragonframe sends an empty bundle or nothing at all when a project has zero axes — only that it does respond when axes exist (empirically confirmed). Rather than guess, a query round-trip is considered "complete" 2.0 seconds after being sent (matching the Signal Monitor's existing `LIVENESS_WINDOW` constant) if no axis has been reported by then; the store transitions from `None` to an empty collection at that point. Any axis response arriving before or after that timeout still updates the store normally — the timeout only governs the never-queried → empty-collection transition, not whether individual axis entries are recorded.
...
| Zero-axes detection mechanism | Timeout-based (2.0s, no response yet) | Assume Dragonframe sends an empty bundle for zero axes | Not empirically confirmed either way — only that Dragonframe responds when axes exist; a timeout doesn't depend on guessing the zero-axes wire behavior |
